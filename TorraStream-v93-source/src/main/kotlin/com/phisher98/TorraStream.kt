...
class TorraStream(private val sharedPref: SharedPreferences) : TmdbProvider() {
    ...
    override suspend fun getMainPage(
        ...
        val languageQuery = if (request.data.contains("language=")) "" else "&language=vi-VN"
        val home = app.get("${request.data}$adultQuery$languageQuery&page=$page")
    ...
    override suspend fun search(query: String, page: Int): SearchResponseList? {
        return app.get(
            "$tmdbAPI/search/multi?api_key=$apiKey&language=vi-VN&query=$query&page=$page&include_adult=${settingsForProvider.enableAdult}"
        ).parsedSafe<Results>()?.results?.mapNotNull { media ->
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        // Genre names are localized to Vietnamese, so use TMDB's stable Animation id.
        val isCartoon = res.genres?.any { it.id == 16 } ?: false
        ...
        return if (type == TvType.TvSeries) {
            val episodes = res.seasons?.amap { season ->
                ...
                app.get("$tmdbAPI/$mediaType/${data.id}/season/${season.seasonNumber}?api_key=$apiKey&language=vi-VN")
    ...
    override suspend fun loadLinks(
        ...
        invokeSubtitleAPI(id, season, episode, isCasting, subtitleCallback)
        return true
    }
