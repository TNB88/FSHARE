...
suspend fun invokeSubtitleAPI(
    ...
    // OpenSubtitles legacy still has a much wider Vietnamese catalog. Its .gz
    // downloads are exposed to the local player as plain SRT by the loopback proxy.
    if (!isCasting) {
        runCatching {
            val numericImdb = id.removePrefix("tt")
            val searchPath = if (season != null && season > 0) {
                "episode-${episode ?: 1}/imdbid-$numericImdb/season-$season/sublanguageid-vie"
            } else {
                "imdbid-$numericImdb/sublanguageid-vie"
            }
            val response = app.get(
                "https://rest.opensubtitles.org/search/$searchPath",
                headers = mapOf(
                    "X-User-Agent" to "VLSub 0.10.3",
                    "User-Agent" to "CloudStream TorraStream Viet"
                )
            )
            val array = JSONArray(response.text)
            for (index in 0 until minOf(array.length(), 15)) {
                val remote = array.optJSONObject(index)?.optString("SubDownloadLink").orEmpty()
                if (!remote.startsWith("https://") || !seen.add(remote)) continue
                val local = VietnameseSubtitleProxy.register(remote) ?: continue
                subtitleCallback(newSubtitleFile("Vietnamese", local))
            }
        }
    }
    ...
    subtitles.sortedByDescending {
        it.lang.equals("vi", true) || it.lang.equals("vie", true) ||
            it.lang.contains("vietnam", true)
    }.forEach { subtitle ->
        val subUrl = subtitle.url
        if (!subUrl.startsWith("http") || !seen.add(subUrl)) return@forEach
        val rawLanguage = getLanguage(subtitle.lang) ?: subtitle.lang.ifBlank { "Phụ đề" }
        val language = if (
            subtitle.lang.equals("vi", true) || subtitle.lang.equals("vie", true) ||
            rawLanguage.contains("vietnam", true)
        ) "Vietnamese" else rawLanguage.replaceFirstChar {
