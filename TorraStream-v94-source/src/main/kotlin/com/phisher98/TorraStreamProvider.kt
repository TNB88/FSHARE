...
class TorraStreamProvider: Plugin() {
    override fun load(context: Context) {
        ...
        val savedMainApisString = sharedPref.getString("main_apis", "TMDB,Anime")
        val savedMainApis = savedMainApisString?.split(",") ?: listOf("TMDB", "Anime")

        // CloudStream stores this preference as a JSON string literal.
        context.getSharedPreferences("rebuild_preference", Context.MODE_PRIVATE)
            .edit()
            .putString("subs_auto_select", "\"vi\"")
            .apply()
