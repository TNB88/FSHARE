...
open class TorraStreamAnime(private val sharedPref: SharedPreferences) : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val data = anilistAPICall(
            "query (\$id: Int = $id) { Media(id: \$id, type: ANIME) { id title { romaji english } startDate { year } genres description averageScore status bannerImage coverImage { extraLarge large medium } bannerImage episodes format nextAiringEpisode { episode } airingSchedule { nodes { episode } } } }"
        ).data.media ?: throw Exception("Unable to fetch media details")

        val anititle = data.getTitle()
        val aniyear = data.startDate.year
        // `format` can be null for incomplete/upcoming AniList records.  The old
        // force unwrap crashed the whole detail page for those titles.
        val mediaFormat = data.format.orEmpty()
        val anitype = if (mediaFormat.contains("MOVIE", ignoreCase = true)) TvType.AnimeMovie else TvType.TvSeries
        ...
        return if (mediaFormat.contains("Movie", ignoreCase = true)) {
            ...
            newAnimeLoadResponse(data.getTitle(), url, TvType.Anime) {
                ...
                // Avoid the host AniListApi.Recommendation model here. Its
                // getter descriptor differs between CloudStream releases and
                // caused NoSuchMethodError on PhimRap/CloudStream 4.8.0.
            }
        }
    }
